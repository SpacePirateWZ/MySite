...
class Mendo_Install_Plugins {
	...
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Main ) ) {
			self::$instance = new Mendo_Install_Plugins();
		}
	...
	public function do_plugin_install(){
		...
		$plugins_title     = array(
			esc_html__('Starter Sites', 'mendo')
		);
		...
		foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
			...
			$links_attrs[$key] = array(
				...
				'data-activating-label' => /* translators: %s: plugin name */ sprintf( __( 'Activating %s', 'mendo' ), $progress_text_and_title ),
				'data-installing-label' => /* translators: %s: plugin name */ sprintf( __( 'Installing %s', 'mendo' ), $progress_text_and_title ),
				'data-activate-label'   => /* translators: %s: plugin name */ sprintf( __( 'Activate %s'  , 'mendo' ), $progress_text_and_title ),
				'data-install-label'    => /* translators: %s: plugin name */ sprintf( __( 'Install %s'   , 'mendo' ), $progress_text_and_title ),
				...
				'data-wpnonce'          => wp_create_nonce( 'mendo-pi_setup_nonce' )
			);
			...
			if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Install %s', 'mendo' ), $progress_text_and_title );
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Activate %s', 'mendo' ), $progress_text_and_title );
			}
		}

	?>
		<?php if ( empty( $not_installed_or_activated_plugins_id ) ) : ?>
		<div class="mendo-message mendo-pi-notice-wrapper mendo-pi-notice-install-now" style="display:inline-block;">
			<a class="button" href="<?php echo esc_url( admin_url( 'themes.php?page=advanced-import.php' ) ); ?>"><?php echo esc_html__( 'Import a demo', 'mendo' ); ?></a>
		</div>		
		<?php else : ?>
		<div class="mendo-message mendo-pi-notice-wrapper mendo-pi-notice-install-now" style="display:inline-block;">
			<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<a class="button button-primary mendo-install-now mendo-pi-not-installed" data-info='<?php echo wp_json_encode( $links_attrs);?>' ><?php echo $links_attrs[0]['button_label']; ?></a>
		</div>
		<?php endif; ?>
	<?php
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'mendo-plugin-install', get_template_directory_uri() . '/assets/js/plugin-install.js', array( 'jquery' ), '', true );

		wp_localize_script(
			'mendo-plugin-install',
			'mendoPluginInstall',
			array(
				'activating' => esc_html__( 'Activating ', 'mendo' ),
			)
		);
	    ...
	    wp_enqueue_script( 'mendo-plugin-install' );

	}
}

$mendo_plugin_install = new Mendo_Install_Plugins();